...
// SWIPER (Library)
let swiper = () => {
  let rec_slider = $(".js-recommendSlider");

  if (rec_slider.length) {
    const rec_swiper = new Swiper(".js-recommendSlider", {
      slidesPerView: "auto",
      speed: 2000,
      autoplay: {
        delay: 3000,
...
const showMore = () => {
  if ($(".home").length) {
    ...
    $(".secArticle .secArticle__item:lt(" + itemNum + ")").attr(
      "data-aos",
      "fade-up"
    );
  ...
  if ($(".archive").length) {
    ...
    $(".secArticle__inner .secArticle__item:lt(" + itemNum + ")").addClass(
      "u-show"
    );
    $(".secArticle__inner .secArticle__item:lt(" + itemNum + ")").attr(
      "data-aos",
      "fade-up"
    );
    AOS.init();
  ...
  if ($(".search").length) {
    ...
    $(".secSearch__content .secSearch__item:lt(" + itemNum + ")").addClass(
      "u-show"
    );
...
const smoothScroll = () => {
  $('a[href^="#"]').click(function () {
    ...
    if (
      !$(this).hasClass("btnMore") &&
      !$(this).hasClass("siteHeader__searchBtn") &&
      !$(this).hasClass("menu") &&
      !$(this).hasClass("btnTop")
    ) {
...
const moveItem = () => {
  if ($(window).width() < 769) {
    $(".u-themethankyou .postDescription").insertAfter(
      $(".u-themethankyou .secSingle__categoryImg")
    );
  } else {
    $(".u-themethankyou .postDescription").insertAfter(
      $(".u-themethankyou .postSec")
    );
  }
};
...
const postRate = () => {
  ...
  $(".u-heartSvg").on("click", function () {
    ...
    $.cookie(contId, 1, { expires: 365, path: "/" });
  ...
  if (
    $(".u-themethankyou").length ||
    $(".u-themeflexible").length ||
    $(".u-themefeature").length
  ) {
    ...
    if ($.cookie(valueID)) {
      setTimeout(function () {
        ...
        $(".feedSecContainer__likeText.hover").html(
          "いいね！" + $(".postSec__postRight .u-heartCount").text()
        );
    ...
    setTimeout(function () {
      $(".postSec__postRight .post-ratings").bind(
        "DOMSubtreeModified",
        function () {
          $(".feedSecContainer__likeText.hover").html(
            "いいね！" + $(".postSec__postRight .u-heartCount").text()
    ...
    setTimeout(function () {
      if ($('body[class*=" paged-"]').length) {
        $(".feedSecContainer__likeText.hover").html(
          "いいね！" + $(".secSingle__contentFlex .u-heartCount").text()
        );
      }
    }, 500);

    // on click feedSecbutton
    $(".feedSecContainer").on("click", function () {
      var curr = $(this);
      //add trigger click here!
      if (!$('body[class*=" paged-"]').length) {
  ...
  var sync = function sync() {
    if (
      ...
      if (ratingBtn02Score.length) {
        ...
        ratingBtn02Icon.after(
          '<p class="ratings-score u-rated">'.concat(
            ratingBtn01ScoreText,
            "</p>"
          )
        );
      }
...
const movePostRating = () => {
  ...
  if (
    ...
    setTimeout(function () {
      $(".secSingle__contentFlex > .post-ratings:first-child").css(
        "display",
        "none"
      );
      $(".feedSecContainer__likeText.hover").html(
...
if ($(".js-kvSlider .swiper-slide").length === 3) {
  kv_swiper.destroy();
  $(".topPageSecKv__btnCont, .topPageSecKv .swiper-pagination").css(
    "display",
    "none"
  );
...
$(document).click(function (event) {
  ...
  if ($(window).width() < 768) {
    ...
    if (
      !$(event.target).closest(".siteHeader__navList").length &&
    ...
    if (
      !$(event.target).closest(".siteHeader__searchCont").length &&
      !$(event.target).closest(".siteHeader__searchBtn").length
    ) {
